...
#   - AWS CLI configured on the host from env-provided credentials (never
#     written to logs, scripts, or git; export AWS_ACCESS_KEY_ID and
#     AWS_SECRET_ACCESS_KEY before running, region defaults to cn-northwest-1)
...
echo "==> [3/6] Copy multica CLI (server-matched version)"
...
echo "==> [4/6] Configure AWS CLI on the runtime EC2 (creds via stdin, never logged)"
if [[ -n "${AWS_ACCESS_KEY_ID:-}" && -n "${AWS_SECRET_ACCESS_KEY:-}" ]]; then
  printf '%s\n%s\n%s\njson\n' "${AWS_ACCESS_KEY_ID}" "${AWS_SECRET_ACCESS_KEY}" "${REGION}" | \
    ssh -i "${TF_DIR}/../../.keys/runtime-kiro.pem" -o StrictHostKeyChecking=no \
      ec2-user@"${PUBLIC_IP}" \
      'umask 077 && mkdir -p ~/.aws && aws configure && chmod 600 ~/.aws/credentials ~/.aws/config'
else
  echo "    SKIP: AWS_ACCESS_KEY_ID / AWS_SECRET_ACCESS_KEY not set (no AWS CLI config written)"
fi

echo "==> [5/6] Configure + authenticate + watch workspace"
ssh -i "${TF_DIR}/../../.keys/runtime-kiro.pem" ec2-user@"${PUBLIC_IP}" \
...
echo "==> [6/6] Install systemd service and start daemon"
ssh -i "${TF_DIR}/../../.keys/runtime-kiro.pem" ec2-user@"${PUBLIC_IP}" \
